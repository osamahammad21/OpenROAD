...
vector<Connection>::iterator connectionFind(vector<Connection>& connections,
                                            Connection& c)
{
...
vector<CommonConnection>::iterator commonConnectionFind(
    vector<CommonConnection>& common_connections,
    CommonConnection& c)
{
...
void Module::addConnectionUtil(Net* net)
{
  ...
  while (term_iter->hasNext()) {
    ...
    if (dir == string("external_driver(input)")) {
      ...
      vector<Instance*>::iterator load_it
          = find(loads.begin(), loads.end(), inst);
  ...
  if (driver_flag) {
    ...
    for (vec_it = loads.begin(); vec_it != loads.end(); vec_it++) {
      ...
      vector<Connection>::iterator vec_connect_it
          = connectionFind(_connections, *c);
    ...
    for (size_t i = 0; i < loads.size(); i++) {
      for (size_t j = i + 1; j < loads.size(); j++) {
        ...
        vector<CommonConnection>::iterator vec_common_connect_it
            = commonConnectionFind(_common_connections, *c);
        if (vec_common_connect_it != _common_connections.end()) {
          vec_common_connect_it->_bundle_net
              = vec_common_connect_it->_bundle_net + 1;
        } else {
    ...
    if ((driver_name != glue_name) || (load_name != glue_name)) {
      ...
      if (map_map_it != _lumped_connections.end()) {
        ...
        if (map_it != map_map_it->second.end()) {
          ...
          map_map_it->second.insert(
              pair<string, LumpedConnection>(load_name, *c));
        }
      } else {
        _lumped_connections.insert(pair<string, map<string, LumpedConnection>>(
            driver_name, map<string, LumpedConnection>()));
        LumpedConnection* c = createLumpedConnection(bundle_net, 0);
        _lumped_connections[driver_name].insert(
            pair<string, LumpedConnection>(load_name, *c));
      }
      ...
      if (map_map_it != _lumped_connections.end()) {
        ...
        if (map_it != map_map_it->second.end()) {
          ...
          map_map_it->second.insert(
              pair<string, LumpedConnection>(driver_name, *c));
        }
      } else {
        _lumped_connections.insert(pair<string, map<string, LumpedConnection>>(
            load_name, map<string, LumpedConnection>()));
        LumpedConnection* c = createLumpedConnection(0, bundle_net);
        _lumped_connections[load_name].insert(
            pair<string, LumpedConnection>(driver_name, *c));
      }
    }
  }
...
void Module::calculateCommonConnectionsUtil()
{
  ...
  for (vec_it = _common_connections.begin();
       vec_it != _common_connections.end();
       vec_it++) {
    ...
    if ((load_a_name != glue_name) || (load_b_name != glue_name)) {
      ...
      if (map_map_it != _lumped_connections.end()) {
        ...
        if (map_it != map_map_it->second.end()) {
          map_it->second._common_fanin
              = map_it->second._common_fanin + bundle_net;
        } else {
          LumpedConnection* c = createLumpedConnection(0, 0, bundle_net);
          map_map_it->second.insert(
              pair<string, LumpedConnection>(load_b_name, *c));
        }
      } else {
        _lumped_connections.insert(pair<string, map<string, LumpedConnection>>(
            load_a_name, map<string, LumpedConnection>()));
        LumpedConnection* c = createLumpedConnection(0, 0, bundle_net);
        _lumped_connections[load_a_name].insert(
            pair<string, LumpedConnection>(load_b_name, *c));
      }
      ...
      if (map_map_it != _lumped_connections.end()) {
        ...
        if (map_it != map_map_it->second.end()) {
          ...
          LumpedConnection* c = createLumpedConnection(0, 0, bundle_net);
          map_map_it->second.insert(
              pair<string, LumpedConnection>(load_a_name, *c));
        }
      } else {
        _lumped_connections.insert(pair<string, map<string, LumpedConnection>>(
            load_b_name, map<string, LumpedConnection>()));
        LumpedConnection* c = createLumpedConnection(0, 0, bundle_net);
        _lumped_connections[load_b_name].insert(
            pair<string, LumpedConnection>(load_a_name, *c));
      }
    }
  }
