...
class Module
{
 ...
 public:
  Module();
  Module(dbVerilogNetwork* network, sta::Instance* inst)
      : _network(network), _inst(inst)
  {
  }
  ...
  std::map<sta::LibertyCell*, int> getTotalMapCells()
  {
    return _total_map_cells;
  }

  std::string getVerilogName() { return std::string(_network->name(_inst)); }

  std::string getPathName() { return std::string(_network->pathName(_inst)); }
  ...
  double getAverageCombFanouts()
  {
    return _total_comb_fanouts * 1.0 / _total_comb_nets;
  }

  // Functions of adding elements
  void addStdCell(sta::Instance* inst) { _std_cells.push_back(inst); }
  ...
  void addTotalMapCell(sta::Instance* inst)
  {
    ...
    std::map<sta::LibertyCell*, int>::iterator map_it
        = _total_map_cells.find(liberty_cell);

    if (map_it != _total_map_cells.end()) {
      map_it->second = map_it->second + 1;
    } else {
      _total_map_cells.insert(
          std::pair<sta::LibertyCell*, int>(liberty_cell, 1));
    }
  }

  void addSubmodule(sta::Instance* inst) { _submodules.push_back(inst); }

  void addMacro(sta::Instance* inst) { _macros.push_back(inst); }
  ...
  void specifyTotalStdCells(int total_std_cells)
  {
    _total_std_cells = total_std_cells;
  }
  void specifyTotalMacros(int total_macros) { _total_macros = total_macros; }
  void specifyTotalCombCells(int total_comb_cells)
  {
    _total_comb_cells = total_comb_cells;
  }
  void specifyTotalSeqCells(int total_seq_cells)
  {
    _total_seq_cells = total_seq_cells;
  }
  void specifyTotalBufInvCells(int total_buf_inv_cells)
  {
    _total_buf_inv_cells = total_buf_inv_cells;
  }
  void specifyTotalCombCellPins(int total_comb_cell_pins)
  {
    _total_comb_cell_pins = total_comb_cell_pins;
  }

  void specifyTotalStdCellArea(double total_std_cell_area)
  {
    _total_std_cell_area = total_std_cell_area;
  }
  void specifyTotalMacroArea(double total_macro_area)
  {
    _total_macro_area = total_macro_area;
  }
  void specifyTotalArea(double total_area) { _total_area = total_area; }
  void specifyTotalCombCellArea(double total_comb_cell_area)
  {
    _total_comb_cell_area = total_comb_cell_area;
  }
  void specifyTotalSeqCellArea(double total_seq_cell_area)
  {
    _total_seq_cell_area = total_seq_cell_area;
  }
  ...
  void specifyStdCellArea(double std_cell_area)
  {
    _std_cell_area = std_cell_area;
  }
  ...
  void printAreaInfo(std::ofstream& file)
  {
    ...
    file << std::setw(40) << "Inv/Buffer Cells" << std::setw(20)
         << _total_buf_inv_cells;
    file << std::setw(20) << numberToString(_total_buf_inv_cell_area)
         << std::endl;
  ...
  void printHierInfo(std::ofstream& file)
  {
    ...
    file << std::setw(25)
         << std::to_string(_total_std_cells) + "/"
                + numberToString(_total_std_cell_area) + " |";
    file << std::setw(25)
         << std::to_string(_total_macros) + "/"
                + numberToString(_total_macro_area) + " |";
    file << std::setw(20)
         << std::to_string(_std_cells.size()) + "/"
                + numberToString(_std_cell_area) + " |";
    file << std::setw(20)
         << std::to_string(_macros.size()) + "/" + numberToString(_macro_area)
                + " |";
    file << std::setw(15) << std::to_string(_submodules.size()) + " |";
    if (_inst == _network->topInstance()) {
      file << std::setw(70) << std::left << std::string(getName()) + " || NULL"
           << std::right << std::endl;
    } else {
      file << std::setw(70) << std::left
           << std::string(getPathName()) + std::string(" || ") + getName()
           << std::right << std::endl;
    }
  }
  ...
  void printLogicalInfo(std::ofstream& file, int level)
  {
    ...
    file << "\"total_area\" : "
         << numberToString(_total_macro_area + _total_std_cell_area) << ","
         << std::endl;
